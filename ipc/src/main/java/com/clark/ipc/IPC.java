...
public class IPC {
    ...
    private static final String TAG=IpcConstant.DEFAULT_TAG+"IPC";
    //服务端
    //==================================================================
    //注册
    public static void regiester(Class<?> service) {
    ...
    public static <T> T getInstanceWithName(Class<? extends IPCService> service, Class<T> instanceClass, String methodName,
                                            Object... parameters) {
        ...
        //服务器响应
        Log.e(TAG, "getInstanceWithName: send("+Request.GET_INSTANCE+",service="+service.getSimpleName()
                +",instanceClass="+instanceClass.getSimpleName()+",methodName="+methodName);
        Response response = Channel.getInstance().send(Request.GET_INSTANCE, service,
                instanceClass, methodName, parameters);
        // response： 成功
        Log.e(TAG, "getInstanceWithName: "+(response==null?"response is null":"response is not null") );
